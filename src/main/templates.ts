...
type ThemeColors = { mainColor: string; accentColor: string }
...
export default defineTheme({
  ...
  blocksDir: "./blocks",
  variables: {
    mainColor: { type: "color", default: "${colors.mainColor}" },
    accentColor: { type: "color", default: "${colors.accentColor}" },
  },
});
`,
...
const blogTemplate: TemplateDefinition = {
  ...
  files: (projectName, themeName) => ({
    ...scaffoldFiles(projectName, themeName, { mainColor: '#1a1a2e', accentColor: '#e94560' }),
    'src/pages/index.mdx': `---
...
const vitrineAssoTemplate: TemplateDefinition = {
  ...
  files: (projectName, themeName) => ({
    ...scaffoldFiles(projectName, themeName, { mainColor: '#2d3436', accentColor: '#0984e3' }),
    'src/pages/index.mdx': `---
layout: ../themes/${themeName}/layouts/Base.astro
title: Accueil
...
export function getTemplates(): TemplateInfo[] {
  return templates.map(({ files: _, ...info }) => info)
}
